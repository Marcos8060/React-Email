...
import LOCTemplate from "../../../emails/template";
  ...
  if (req.method === "GET") {
    try {
      ...
      const { data: emailAlertsData } = await axios.get(
        `${API_URL.LOC_EMAIL}?api_key=${process.env.NEXT_PUBLIC_API_KEY}`
      );


      // Generate HTML content using the template component
      const emailBody = ReactDOMServer.renderToString(
        <LOCTemplate emailAlertsData={emailAlertsData} />
      );
      ...
      await sendEmail({
        to: [
          ...
          `${emailAlertsData[0]?.recipientemailaddress}`,
